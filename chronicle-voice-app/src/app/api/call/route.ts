...
export async function POST(request: Request) {
    try {
        ...
        console.log("Backend received call request", phoneNumber ? `to ${phoneNumber}` : "to default number");
        ...
        if (!customerNumber) {
            return NextResponse.json(
                { success: false, message: "No phone number provided and no default set" },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: false, message: `Failed to process call request: ${errorMessage}` },
            { status: 500 }
        );
    }
}
